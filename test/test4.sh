...
echo "#N\texact\tcomputed" > ./error/global_error_4.dat
...
do
	...
	# get computed energy
	computed=$(./harmonic.x ./input/"$filename" 4 | tail -1 | cut -d " " -f 12)

	# write to energy file
	line="$N\t3.5\t$computed"
	echo $line >> ./error/global_error_4.dat

	# fix output file
	cp res* ./output/4/$filename
	rm res*
