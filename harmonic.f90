...
!---------------------------------------------------------------
  !
  !       Code for the exam of Numerical Methods in
  !       Quantum Mechanics, adapted by Andrea Mecchina
  !       from an original code by professor Paolo Giannozzi.
  !
  ...
  !
  integer :: mesh, i, j, icl, ios, input_flag
  integer :: nodes, hnodes, ncross, kkk, n_iter
  real(dp) :: xmax, dx, ddx12, norm, arg, djump, fac
  real(dp) :: eup, elw, e, de, dfcusp, ycusp
  real(dp), allocatable :: x(:), y(:), p(:), vpot(:), f(:)
  character (len=80) :: filein, fileout, ord, p_flag
  !
  ! read input data
  !
  if (command_argument_count() == 0) then
     ...
     read (*,'(a)') fileout
     print '(a,$)','Expansion order around inversion point (default 3) > '
     read (*,'(a)') ord
     print '(a,$)','Perturbative estimate energy gap? (1 for Yes) > '
     read (*,'(a)') p_flag
  else
     input_flag = 1
     !
     ...
     call get_command_argument(2, ord)
     call get_command_argument(3, p_flag)
...
search_loop: do 
  ...
  iterate: do kkk = 1, n_iter
     ...
     if (p_flag == '1') then
        !
        ! Perturbative estimate energy gap
        ...
        if ( n_iter > 1 ) then
           ...
           !  y'(i;RIGHT) - y'(i;LEFT)
           if (ord == '2') then
              djump = (y(icl+1) + y(icl-1) - 2*y(icl)) / dx
           else if (ord == '4') then
              djump = ( y(icl+1) + y(icl-1) - (14.0_dp-12.0_dp*f(icl))*y(icl) ) / (dx*(3-2*f(icl)))
           else
              djump = ( y(icl+1) + y(icl-1) - (14.0_dp-12.0_dp*f(icl))*y(icl) ) / dx         
           end if
           print '(i5,f25.15,i5,f14.8)', kkk, e, nodes
